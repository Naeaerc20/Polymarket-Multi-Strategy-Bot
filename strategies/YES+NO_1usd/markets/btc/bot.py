"""
strategies/YES+NO_1usd/markets/btc/bot.py
----------------------------------
...
.env variables:
  BTC_PRICE_RANGE        e.g. "0.40-0.45"  — range for standard buy (both sides if LP=false, second side if LP=true)
  BTC_TRIGGER_RANGE      e.g. "0.52-0.54"  — range for first-side buy (only used when LOSS_PREVENTION=true)
  BTC_AMOUNT_TO_BUY      USDC amount per side (e.g. 1.0)
  BTC_POLL_INTERVAL      seconds between price checks (e.g. 0.5)
  BTC_LOSS_PREVENTION    true | false  (default: false)
...
_trigger_raw      = os.getenv("BTC_TRIGGER_RANGE", "0.52-0.54")
_lp_raw           = os.getenv("BTC_LOSS_PREVENTION", "false")
