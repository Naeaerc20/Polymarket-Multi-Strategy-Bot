"""
strategies/RSI_VWAP_Signal/markets/eth/bot.py
------------------------------------------------------
RSI + VWAP Signal Strategy — ETH
...
2. CONNECT BINANCE WEBSOCKET
   Opens a persistent stream to wss://stream.binance.com for ETHusdt
...
3. CONNECT POLYMARKET WEBSOCKET (MarketStream)
   ...
   current ETH 5m window. Gets real-time UP and DOWN token prices.
...
.env variables (ETH-specific):
  ETH_RSI_VWAP_PRICE_RANGE    Price range to buy token e.g. "0.30-0.55"
  ETH_RSI_VWAP_AMOUNT         USDC per buy e.g. 1.0
  ETH_RSI_VWAP_POLL_INTERVAL  Seconds between price checks (default 0.5)
...
log = logging.getLogger("ETH-RSI-VWAP")
...
# Per-asset
PRICE_RANGE    = _parse_range(os.getenv("ETH_RSI_VWAP_PRICE_RANGE", "0.30-0.55"))
AMOUNT_TO_BUY  = _float("ETH_RSI_VWAP_AMOUNT",        1.0)
POLL_INTERVAL  = _float("ETH_RSI_VWAP_POLL_INTERVAL", 0.5)
...
class BotState:
    ...
    def summary(self) -> str:
        lines = ["ETH RSI+VWAP position summary:"]
        if self.bought_up:
            sig = self.up_signal
            lines.append(
...
def run(interval: Optional[str] = None):
    ...
    range_low, range_high = PRICE_RANGE
    log.info("=" * 60)
    log.info("ETH RSI+VWAP Strategy starting")
