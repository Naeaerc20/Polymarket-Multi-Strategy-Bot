"""
strategies/RSI_VWAP_Signal/markets/xrp/bot.py
------------------------------------------------------
RSI + VWAP Signal Strategy — XRP
...
2. CONNECT BINANCE WEBSOCKET
   Opens a persistent stream to wss://stream.binance.com for XRPusdt
...
3. CONNECT POLYMARKET WEBSOCKET (MarketStream)
   ...
   current XRP 5m window. Gets real-time UP and DOWN token prices.
...
.env variables (XRP-specific):
  XRP_RSI_VWAP_PRICE_RANGE    Price range to buy token e.g. "0.30-0.55"
  XRP_RSI_VWAP_AMOUNT         USDC per buy e.g. 1.0
  XRP_RSI_VWAP_POLL_INTERVAL  Seconds between price checks (default 0.5)
...
log = logging.getLogger("XRP-RSI-VWAP")
...
# Per-asset
PRICE_RANGE    = _parse_range(os.getenv("XRP_RSI_VWAP_PRICE_RANGE", "0.30-0.55"))
AMOUNT_TO_BUY  = _float("XRP_RSI_VWAP_AMOUNT",        1.0)
POLL_INTERVAL  = _float("XRP_RSI_VWAP_POLL_INTERVAL", 0.5)
...
class BotState:
    ...
    def summary(self) -> str:
        lines = ["XRP RSI+VWAP position summary:"]
        if self.bought_up:
            sig = self.up_signal
            lines.append(
...
def run(interval: Optional[str] = None):
    ...
    range_low, range_high = PRICE_RANGE
    log.info("=" * 60)
    log.info("XRP RSI+VWAP Strategy starting")
