...
-----------------------------------------------
ETH Up/Down Bot for Polymarket — DCA Snipe strategy.
...
ETH_BET_STEP           null | float — DCA step
ETH_POLL_INTERVAL      seconds between ticks

INTERVAL (runtime menu/argument):
  5m    | 15m | 1h    → timestamp-slug markets (e.g. eth-updown-1h-1740560400)
  1h_et              → ET-dated hourly  (e.g. ethereum-up-or-down-february-26-10am-et)
  24h                → daily market     (e.g. ethereum-up-or-down-on-february-26)
...
ENTRY_PRICE      = _float_env("ETH_ENTRY_PRICE",    0.70)
AMOUNT_PER_BET   = _float_env("ETH_AMOUNT_PER_BET", 1.0)
TAKE_PROFIT      = _float_env("ETH_TAKE_PROFIT",    0.95)
POLL_INTERVAL    = _float_env("ETH_POLL_INTERVAL",  0.5)
BET_STEP         = _optional_float("ETH_BET_STEP")
STOP_LOSS        = _optional_float("ETH_STOP_LOSS")
STOP_LOSS_OFFSET = _optional_float("ETH_STOP_LOSS_OFFSET")
USE_STOP_LOSS    = os.getenv("ETH_USE_STOP_LOSS", "true").strip().lower() not in ("false", "0", "no")
...
def run_window(market: dict, executor: OrderExecutor, state: BotState, interval: str):
    ...
    log.info("=" * 60)
    log.info(f"  ETH DCA | Market: {market.get('id','')}")
...
def run(interval: Optional[str] = None):
    ...
    log.info("=" * 60)
    log.info("ETH DCA Snipe starting")
    log.info(f"  Interval : {interval.upper()}")
    log.info(f"  ENTRY={ENTRY_PRICE}  BET=${AMOUNT_PER_BET}  TP={TAKE_PROFIT}")
